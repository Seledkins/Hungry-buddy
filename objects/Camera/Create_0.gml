...
#region camera scale change

var arena_width = global.arena_width_scale * global.sprite_arena_width;
var arena_height = global.arena_height_scale * global.sprite_arena_height;
...
if (flag_to_scale_h || flag_to_scale_v)
{
	while(view_width - margin > arena_width && view_height - margin > arena_height){
		
		view_width /= scale_acceleration;
		view_height /= scale_acceleration;
		
		show_debug_message($"Width of camera resized to {view_width} ------- arena width: {arena_width}");
		show_debug_message($"Height of camera resized to {view_height} ------- arena height: {arena_height}");
	}
...
x = room_width/2; // Adjust the camera to the top left corner
y = room_height/2;
