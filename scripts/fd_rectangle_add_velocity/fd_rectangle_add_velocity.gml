...
function fd_rectangle_add_velocity(rectangle_id, _sprite_index, _image_index, _x, _y, xscale, yscale, x_velocity, y_velocity) {
	...
	with (rectangle_id) {
	    fd_rectangle_set_target(id, FD_TARGET_TYPE.ADD_VELOCITY);
	        var color = make_color_rgb(ceil((clamp(x_velocity, -1, 1) * 0.125 + 0.5) * 255), ceil((clamp(y_velocity, -1, 1) * 0.125 + 0.5) * 255), 0);
	        draw_sprite_ext(_sprite_index, _image_index, _x, _y, xscale, yscale, 0, color, 1);
